...
`.github/workflows/scout.yml` сам не запускается. Он висит на кнопке: вкладка
Actions → gravel scout → Run workflow. Прогоняет тесты, обходит источники и
коммитит `data/seen.json` обратно в репозиторий; issue и телеграм по умолчанию
выключены, включаются галочкой при запуске.

Расписание убрано намеренно. С адресов GitHub доступна одна доска из пяти —
остальные четыре отдают Cloudflare-челлендж, — так что фоновый прогон делал
малую часть работы телефонного и при этом сам засорял список issues. Полный
обход живёт на телефоне, см. «Локальный прогон» выше.
